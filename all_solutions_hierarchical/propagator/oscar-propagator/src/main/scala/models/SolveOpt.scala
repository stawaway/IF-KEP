...
class ProblemInstance(fileName: String, verbose: Boolean = false) {
  ...
  def getBackArcs(cycles: Array[Array[Int]]): Array[Int] = {
    val numCycles = cycles.length
    var backArcsCount = Array.fill(numCycles)(0)
    ...
    for (i <- 0 until numCycles) {
      ...
      for (a <- cycle) {
        for (b <- cycle) {
          if (a != b && neighbors(a).contains(b) && neighbors(b).contains(a)) {
            numBackArcs = numBackArcs + 1
          }
        }
      }
      backArcsCount(i) = numBackArcs
    }
    backArcsCount
  }
}
...
class KepHierarchicalSolver(problem: ProblemInstance, cycleLimit: Int) {
  def solveKEP: (Array[Array[Int]],Array[Int],Int,Int,Int) = {
    ...
    val cycles = problem.getCycles(cycleLimit)
    val backArcs = problem.getBackArcs(cycles)
    val cycleVars = Array.fill[IloNumVar](cycles.length)(null)
    val includedIn = Array.fill(problem.numVertices)(ListBuffer[Int]())
    for (c <- 0 until cycles.length) {
      val cycle = cycles(c)
    ...
    var objExpr = cplex.linearNumExpr()
    for (i <- 0 until cycles.length) {
      val x = cplex.numVar(0.0, 1.0, IloNumVarType.Bool)
      cycleVars(i) = x
      objExpr.addTerm(cycles(i).length, x)
    ...
    cplex.solve
    val numPatients = cplex.getObjValue.toInt

    cplex.addGe(objExpr, cplex.getObjValue.toInt)
    objExpr = cplex.linearNumExpr()
    for (i <- 0 until cycles.length) {
      objExpr.addTerm(1.0, cycleVars(i))
    }
    cplex.getObjective.setExpr(objExpr)
    cplex.solve
    val numCycles = cplex.getObjValue.toInt

    cplex.addGe(objExpr, cplex.getObjValue.toInt)
    objExpr = cplex.linearNumExpr()
    for (i <- 0 until cycles.length) {
      objExpr.addTerm(backArcs(i), cycleVars(i))
    }
    cplex.getObjective.setExpr(objExpr)
    cplex.solve
    (cycles, backArcs, numPatients, numCycles, cplex.getObjValue.toInt)
  }
}
