...
class KepCover(
  ...
  private[this] var coverConstraint: IloRange = null
  ...
  def setupModel(): Unit = {
    ...
    val coverExpr = cplex.linearNumExpr()
    for (i <- 0 until cycles.length)
      coverExpr.addTerm(1.0, cycleVars(i))
    coverConstraint = cplex.addGe(coverExpr, 0.0);
  ...
  def updateMustCover(mustCoverIndices: Array[Int]): Unit = {

    val coverConstraintCoefs = Array.fill[Double](nCycles)(0.0)

    for (index <- mustCoverIndices)
      for (value <- X(index).min to X(index).max)
        if (value != index && X(index).hasValue(value)) {
          val includingCycles = edgeToCycles.getOrElse((index, value), null)
          for (c <- includingCycles)
            coverConstraintCoefs(c) = 1.0
        }
    
    coverConstraint.setUB(1.0)
    cplex.setLinearCoefs(coverConstraint, coverConstraintCoefs, cycleVars)
